...
Adapters shipped: `DuckDbTableStore`, `DeltaTableStore` (delta-rs), `PostgresTableStore`,
...
pulls `aiokafka`; `s3` pulls `boto3`. The base install of `twinflow-storage` has only
