...
compose, (c) historian naming, (d) alarm prioritization and rationalization, (e) OPC UA bridge,
...
| CON-5  | IP hygiene: zero client artifacts, employer code, internal documents, or proprietary data | P0, VAL-GATE-IPH-001                               |
...
**Owned artifacts:** `ROADMAP.md`, `roadmap.yaml`, `requirements.yaml`, `splits.yaml`,
...
**Purpose.** Program management as code. It parses a machine-readable roadmap, proves the
dependency graph is a valid DAG, proves every requirement in the source prompt is placed exactly
once, renders `ROADMAP.md` and the Mermaid dependency graph, synchronizes GitHub milestones and
...
it after a step that writes README. Scoping the cleanliness check to the three roadmap artifacts
...
Five entities. All are Pydantic models, all serialize to and from `roadmap.yaml` and
...
| `deliverables` | `list[str]`                                           | Concrete artifacts: module path, config key, CLI command, doc page                |
...
| `status`       | `Literal["planned","in_progress","done","reordered"]` | No `canceled`, no `wontfix`                                                       |
...
|----------------|--------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| `validation`   | `reference`, `reference_url`, `assertion`, `falsified_by`                | Agreement with an artifact published outside this repository                      |
...
not visible in the artifact a reader opens is a gate nobody audits.
...
`ops.workpackage.canceled` event type, and a CI policy test asserts that no such type is ever
...
## 5. Behavior: the sequencing argument
...
Retrofitting a contract does not merely cost rework, it invalidates every artifact produced
...
- **C1 determinism.** The seed manifest, splittable RNG topology, and per-subsystem child seed
  ...
  field is serialized as the shortest decimal string that round-trips to the same IEEE-754
  binary64 value, which makes the serialization a function of the value alone. That fixes what
...
| `VAL-GATE-A11Y-001`   | validation   | axe-core reports zero critical or serious violations, the demo path is completable by keyboard alone, severity is encoded by shape and text and not by color alone, and reduced-motion is honored (C12)                                                                                                                                  |
...
| `VAL-GATE-IPH-001`    | policy       | The banned-terms pre-commit hook is installed and its CI counterpart runs on every pull request; the generic organization-name check reports zero unreviewed hits across the tree (CON-5)                                                                                                                                                |
...
that is where the public artifact set is complete and the package APIs, REST/MCP contracts, and
event schemas stabilize enough for C9's compatibility promises to mean something.
...
| **P3d Planning**                            | v0.7.0     | Component 6a (demand signal, forecasting arena with interval contract, inventory optimization, ABC/XYZ, SIOP feedback into truck scheduling), **E12 yard and dock optimization (tail)**                                                                                                                                                                                                                                                                                                                  | P3c                   | E9                      | 3e onward, 3g, E15, E16, E23, E31    | FCST-001, FCST-002, INV-001, YARD-001                                                                                                          |
...
| **P3g Cross-dock and e-commerce**           | v0.10.0    | Component 6a5 cross-docking, component 6a6 e-commerce fulfillment                                                                                                                                                                                                                                                                                                                                                                                                                                        | P3e                   | E12                     | 6a12, E45                            | XDOCK-001, CART-001                                                                                                                            |
...
| **ROST Rostering**                          | v0.16.0    | E23 labor rostering optimization                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 6a10, P3d             | nothing                 | 6a12, 6a14                           | ROST-001                                                                                                                                       |
...
| **P5 Polish and protocol**                  | **v1.0.0** | Component 9c README final with the limitations section, demo GIF refresh, capability report artifact, RA(e) OPC UA bridge, mTLS from the internal CA, A5 final, A2b and A2d proven against the demo, A3c Helm chart, A6c GraphQL, C9 stable API promise                                                                                                                                                                                                                                                  | P4                    | nothing                 | P6                                   | MTLS-001, OPCUA-001, ADOPT-001                                                                                                                 |
...
| P3d   | Demand signal, forecasting arena                   | Interval contract, inventory optimization     | ABC/XYZ, SIOP feedback                      | E12                             |
...
| R11  | E9 -> **head of P3b**                                                       | Line 33's slotting is "optimized on velocity, cube, affinity, and ergonomics", which is a search over assignments. Component 7's `compare_scenarios` ranking is the same harness. Two ad hoc searches now, or one optimizer package (`twinflow-optimize`) reused by E12, E20, E42, and slotting                                                                                                                                                                                                                                                                                                                                         |
...
| R04  | E26(b) governed metrics layer -> **P0**                                     | C5 (line 136) lists "metrics layer" among the configs that must validate against a published schema at load, which places it at the config layer, before any consumer. Independently: component 1 computes takt, cycle time, WIP, utilization, and OEE from P1. If those are computed inline and later moved into a MetricFlow-style YAML, every published number changes definition and every earlier artifact is wrong                                                                                                                                                                                                                |
...
| R45 | E25         | **E25a** export framework, dataset card schema, license recording (P3c); **E25b** full corpus across every labeled phenomenon (P6-W6, fed continuously)                                                                                                                                                                                                                                                                                                                                                                                         |
...
|-----|--------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CD1 | E23 rostering needs predicted absenteeism; 6a14 HR produces it                                         | `AbsenteeismModel` protocol                                                                                   | E23 ships `ConfiguredRateAbsenteeism` reading `workforce.absenteeism_rate` from facility.yaml                                                                                         | 6a14 ships `BehavioralAbsenteeism` driven by overtime, strain, and schedule stability, and the E23 gate re-runs                                                                                                                     |
| CD2 | 1b slotting optimizes on ergonomics; 6a10 defines the ergonomic index                                  | `ErgonomicScore` protocol                                                                                     | P3b ships `HeightWeightPenalty`, a static rule                                                                                                                                        | 6a10 ships `NioshLiftingIndexScore`, and a regression test asserts the slotting ranking changes and records by how much                                                                                                             |
...
| CD6 | E43 registry needs models; models need a registry                                                      | Registry first, empty                                                                                         | P3 registry ships with the PdM baseline registered as model zero                                                                                                                      | Every later model registers at creation; a CI check fails if a model artifact exists outside the registry                                                                                                                           |
...
|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| 1          | P1 (one station), P2 (takt, cycle time, WIP, utilization, OEE, bottleneck), P3c (VSM summary)                                                                      | The metrics resolve through E26(b) from first computation                                                                                          |
...
| RA-d alarm prioritization, dedupe, severity ranking, shelving            | P3                          | Flood risk starts when 50+ devices publish into SPC                                                                                                                                          |
...
2. Deliverables as a task list, one checkbox per artifact (module path, config key, CLI command,
   doc page).
...
phases:
  - id: P3d
    ...
    delivers: >
      ...
      optimization, ABC/XYZ segmentation, SIOP feedback into truck scheduling,
      and E12 yard and dock optimization.
...
work_packages:
  - id: WP-P3d-04
    title: Optimize inbound slotting and dock-door assignment against the twin
    phase: P3d
    wave: 4
    covers:
...
- `test_validation_gate_without_reference` asserts a `kind: validation` gate with an empty
  `reference` is a load error. This is the mechanized form of the hard rule that statistical
  claims name their source.
- `test_status_enum_has_no_cancellation_value` asserts the literal set of allowed status values
  is exactly `{planned, in_progress, done, reordered}`. If someone adds `canceled`, this fails.
...
A `validation` gate names an artifact published outside this repository, with edition and
locator, and its tolerance is never tighter than the precision that artifact prints. A
...
|--------------------|--------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|-------|
| `VAL-GATE-MTB-001` | Minitab p-chart documentation example                        | Proportions, center line, and the variable control limits match every printed digit to half a unit in the last place                                       | P2    |
...
| Gate              | Reference                                         | Assertion                                                                                                                                                                                                                                                                                                                                                                                                                                               | Phase |
|-------------------|---------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------|
| `VAL-GATE-RF-001` | Friis transmission equation, free-space path loss | Computed received power matches the closed-form Friis result to a relative error of 1e-9 at 20 sample geometries, and read probability is monotone non-increasing in distance and monotone in tag orientation angle. There is no public certified UHF RFID read-rate dataset, so the physics core is validated against the closed form and the behavior against invariants, and the registry says so rather than implying a dataset that does not exist | P2    |
...
artifact, which is why none of them carries a reference field.
...
| `VAL-GATE-OPT-001`         | invariant | The optimizer reproduces its best configuration exactly when re-run at the same seed, and its reported best is never worse than the best evaluated trial                                                                                                                                                                                                                                                                                                                                                                                                                | P3b   |
...
| `VAL-GATE-ABSENT-SEAM-001` | invariant | The same pattern for CD1: the rostering solution under the configured rate differs measurably from the solution under the behavioral predictor                                                                                                                                                                                                                                                                                                                                                                                                                          | 6a14  |
...
| `WP-P6-W6-01` Final coverage proof, `VAL-GATE-COVER-001`, published dependency graph as a repo artifact                                                                                                                                                                                                                       | P6-W6 | The closing proof that every one of the source's requirements shipped                                                                                                                                                                                                                                                 |
...
1. **"All ten are committed roadmap milestones for Phase 6" (line 72) precedes a list of 48
   items.** This reads as a drafting artifact from an earlier, shorter list. This section assumes
...
10. **E47's hardware dependency versus CI.** E47 requires a physical ESP32. CI cannot have one.
    ...
    CI covers the enrollment, publish, and OTA logic, and the physical demo is a recorded
    artifact with its own provenance note. Confirm that a recorded artifact satisfies the
