...
`twinflow.dashboard` and `twinflow.api` side by side in the `apps` tier, and
...
SOURCE_ROOT = PACKAGE_ROOT / "src" / "twinflow" / "dashboard"

#: The sibling in the same layer. R32: this package reads that surface over
#: HTTP from the browser, and an import in either direction is the shortcut
#: that makes these tests untrue of the deployed system.
SIBLING_APP = "api"
...
def test_package_imports_only_its_declared_workspace_dependencies():
    ...
    for path in sorted(SOURCE_ROOT.rglob("*.py")):
        borrowed = _workspace_imports(path) - {"dashboard"} - allowed
        if borrowed:
...
def test_no_module_here_imports_the_other_app_in_this_layer():
    ...
    assert offenders == set(), (
        ...
        f"Requirement R32: this package reads that surface over HTTP from the browser, "
        f"and an import either way makes these tests untrue of the deployed system. "
